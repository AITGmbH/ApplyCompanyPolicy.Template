...
REM cls
set nuget_packages_dir=packages
set paket_packages_dir=packages
...
if exist packages.config (
  if exist %nuget% (
    ...
    "%nuget%" "install" "packages.config" "-OutputDirectory" %nuget_packages_dir% "-ExcludeVersion"
    if %ERRORLEVEL% NEQ 0 exit /b %ERRORLEVEL%
  ) else (
...
if not exist %fake% (
  set fake=%nuget_packages_dir%/%fake_path%
  if not exist %fake% (
